...
add-highlighter shared/ regions -default code django-template \
    scope '\{%' '%\}' '' \
    expansion '\{\{' '\}\}' '' \
    comment '\{#' '#\}' ''
...
add-highlighter shared/django-template/comment fill comment
...
hook global WinSetOption filetype=django-template %{
    add-highlighter window ref django-template
}
hook global WinSetOption filetype=(?!django-template).* %{
