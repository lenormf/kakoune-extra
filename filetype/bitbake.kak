...
add-highlighter shared/ regions -default code bitbake \
    double_string  %{(?<!\\)(\\\\)*\K"} %{(?<!\\)(\\\\)*"} '' \
    single_string %{(?<!\\)(\\\\)*\K'} %{'} '' \
    comment '(?<!\$)#' '$' '' \
    python_inline '\$\{@' \} '' \
...
add-highlighter shared/bitbake/python_inline ref python
add-highlighter shared/bitbake/python_bb ref python
add-highlighter shared/bitbake/python_native ref python
add-highlighter shared/bitbake/python_bb regex '^\h*(python)(\h+\w+)?\h*\(\h*\)\h*\{' 1:green+b

add-highlighter shared/bitbake/shell_bb ref sh
add-highlighter shared/bitbake/double_string ref sh/double_string
add-highlighter shared/bitbake/single_string ref sh/single_string
add-highlighter shared/bitbake/comment ref sh/comment
...
hook global WinSetOption filetype=bitbake %{
    add-highlighter window ref bitbake
}
hook global WinSetOption filetype=(?!bitbake).* %{
